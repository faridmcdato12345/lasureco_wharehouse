...
<div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0 text-dark">Sitio Voucher</h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
            ...
            <div class="voucher-div">
                <div class="voucher-list" id="voucher-list">
                    ...
                    {{-- <form action="" method="post">
                        <button type="submit" class="btn btn-primary form-control saveToSitio">SAVE</button>
                    </form> --}}
                    <button class="btn btn-primary form-control saveToSitio">SAVE & PRINT</button>
                </div>
            </div>
        </div>
      </div>
...
<script>
    $(document).ready(function() {
        ...
        function generateVoucherCode(){
            ...
            $('.voucher-number').html('SI-'+ voucherCode)
        }
        var table = $('.data-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('sitio.index') }}",
            columns: [
                {data: 'id', name: 'id'},
        ...
        $('body').on('click','.addToSitioVoucher',function () {
        ...
        $('body').on('click','.saveToVoucher',function (e) {
            let urlUpdate = "{{route('sitio.update',':id')}}";
        ...
        //save to Sitio database
        $('body').on('click','.saveToSitio',function(){
            generateVoucherCode()
            let productid = '';
            let quantity = 0;
            ...
            $("tr.item").each(function() {
                ...
                sendToSitioDb(voucher_id,productid,voucherNo,quantity,place,description_of_work,mctNo,issued_by,received_by,order_number,order_type)
        ...
        function undoTheQuantity(id,req){
            let urlUpdate = "{{route('sitio.undo_material',':id')}}";
        ...
        function subtractQuantity(id){
            let urlUpdate = "{{route('sitio.subtract_material',':id')}}";
            urlUpdate = urlUpdate.replace(':id',id);
            $.ajax({
                data: {
                    return_quant: 1,
        ...
        function sendToSitioDb(voucher_id,material_id,request_voucher,quantity,place,description_of_work,mct_number,issued_by,received_by,order_number,order_type){
            let id = voucher_id
            let code = request_voucher
            let url = "{{url('/voucher/sitio/print/4/:voucherCode')}}";
